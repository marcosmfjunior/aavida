...
class Post
{
    public function getUltimoId()
    {
        ...
        $consulta = $conexao->prepare('SELECT id_post FROM post WHERE id_blog = 1 ORDER BY data DESC LIMIT 1');
    ...
    public function getTodosPosts($idBlog)
    {
        $conexao = BancoDados::getInstancia();
        //nunca use * em projetos de medio-grande porte
        $consulta = $conexao->prepare('SELECT * FROM post WHERE id_blog = ? ORDER BY data ');
        $consulta->bindValue(1,$idBlog,\PDO::PARAM_INT);
        $resposta = $consulta->execute();
        if(!$resposta){
            echo 'Problema ao buscar noticia '.$idBlog;
